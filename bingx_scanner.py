...
KLINES_LIMIT: int = 150              # Количество свечей для расчёта EMA и MACD

# --- ПОРОГИ ОСНОВНЫХ ФИЛЬТРОВ ---
OI_THRESHOLD: float = 1_000_000.0    # Минимальный Open Interest в USDT
OI_CHANGE_PERCENT: float = 10.0      # Минимальный рост OI в процентах (если используется)
...
# --- РЕЖИМЫ РАБОТЫ ---
USE_OI_CHANGE: bool = False          # True = процентное изменение OI, False = абсолютный порог OI
                ...
                for symbol in passed_funding:
                    try:
                        ...
                        if USE_OI_CHANGE:
                            ...
                            if oi_change is not None and abs(oi_change) >= OI_CHANGE_PERCENT:
                                oi_passed = True
